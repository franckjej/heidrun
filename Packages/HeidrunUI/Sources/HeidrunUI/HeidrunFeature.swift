...
public protocol HeidrunFeature {
    ...
    /// Whether the host should display the user-list inspector beside this
    /// feature. Defaults to `false`; only features that depend on the
    /// connected-users list (e.g. Chat) opt in to seeing the detail
    /// column.
    static var wantsUserListInspector: Bool { get }
}

public extension HeidrunFeature {
    static var wantsUserListInspector: Bool { false }
}
